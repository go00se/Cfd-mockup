...
# ALL CODE BELOW IS FOR MOCKUP ONLY, NOT FINAL IMPLEMENTATIONS

def plot_streamlines_obstacles(X, Y, U, V, obstacles=None, title="Flow with Obstacles"):
    ...
    if obstacles:
        for obs in obstacles:
            if obs ['type'] == 'circle':
                circle = plt.Circle((obs['x'], obs ['y']), obs['radius'], color='red', alpha=0.7, zorder=10)
                ax.add_patch(circle)
